...
export default function SuperAdminDashboard({ reports, totalReports, adminRole }: DashboardProps) {
  const { locale } = await getCopy();
  const t = (key: keyof typeof t) => (locale as any)[key];
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="flex gap-2">
          {adminRole === "super_admin" && (
            <button onClick={() => console.log("View all reports")} className="px-4 py-2 bg-blue-600 text-white rounded">View All Reports</button>
          )}
          {adminRole === "super_admin" && (
            <button onClick={() => console.log("Export reports")} className="px-4 py-2 bg-green-600 text-white rounded">Export Reports</button>
          )}
        </div>
      </div>
